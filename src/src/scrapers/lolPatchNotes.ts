...
export const scraper = {
    feedName: "League Of Legends", // *mandatory
    feedURL: 'https://www.leagueoflegends.com/en-us/news/tags/patch-notes/', // *mandatory
...
export async function getLastArticle(): Promise<jsonArticle | null> {
    ...
    const article = responseData('a[data-testid="articlefeaturedcard-component"]').first();
    if (!article.length) {
        console.error('[LOL] Article grid not found');
        return null;
    }

    const articleAttr = article.attr();
...
export async function getLastUpdate(): Promise<jsonItem | null> {
    ...
    const articleAttr = article.attr();
    const articleURL = new URL(articleAttr?.href!, scraper.feedURL).toString();
    const articleTitle = articleAttr!['aria-label'];
...
export async function getSpecificUpdate(updateName: String): Promise<jsonItem | null> { // A faire
    ...
    const article = responseData(`a[aria-label="${updateName}"]`);
    if (!article.length) {
        console.error('[LOL] Article grid not found');
        return null;
    }
