...
export const data = new SlashCommandBuilder()
    .setName('unsubscribe')
    .setDescription('Unsubscribe to a feed')
    .addStringOption(option =>
        option.setName('feedname')
            .setDescription('feed name to unsubscribe')
            .setRequired(true)
...
export async function execute(interaction: ChatInputCommandInteraction) {
    ...
    if (isPM) {
        ...
        if (!subscriptions[feedName]["user"].includes(userID)) {
            return await interaction.editReply(`❌ You are not subscribe to **${feedName}** here.`);
        }

        const index = subscriptions[feedName]["user"].indexOf(userID);
        subscriptions[feedName]["user"].splice(index, 1);
        fs.writeFileSync(subscriptionsPath, JSON.stringify(subscriptions, null, 2));

        await interaction.editReply(`✅ You are now unsubcribe of **${feedName}**.`);
        ...
        const index = subscriptions[feedName]["channel"].indexOf(channelID);
        subscriptions[feedName]["channel"].splice(index, 1);
        fs.writeFileSync(subscriptionsPath, JSON.stringify(subscriptions, null, 2));

        await interaction.editReply(`✅ Channel unsubscribe to **${feedName}** !`);
    }
}